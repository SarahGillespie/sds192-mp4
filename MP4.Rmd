...
```{r makeAGGPLOT, message=FALSE, warning = FALSE}
...
bike_icon <-makeIcon("bike_icon.png")
```
...
leaflet(bikeStations) %>%
  addTiles() %>%
  addMarkers(~bikeStations$lon, ~bikeStations$lat, popup = bike_icon) %>%
  addCircles(~bike_crashes_tidy$LONGITUDE, bike_crashes_tidy$LATITUDE, weight = 2, radius = 40,
